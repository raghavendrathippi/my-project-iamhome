...
public class NativeGeocoder {
	...
	public static String DaumMapAPI = "http://apis.daum.net/maps/addr2coord?apikey="
			+ DaumMapKey;

	// EXAMPLE :
	// http://map.naver.com/api/geocode.php?key=8c36c72309be8ab6abd5d527cb472e0f&encoding=utf-8&coord=tm128&query=blahblah
	...
	public static String NaverMapAPI = "http://map.naver.com/api/geocode.php"
			+ "?" + NaverMapKey + "&" + NaverEcodingType + "&" + NaverCoordType;
	...
	public static Address getGeocodedAddress(String address) {
		...
		try {/*
			 * mURI = DaumMapAPI + "&q="+address+ "&output=xml"; mHttpGet = new
			 * HttpGet(mURI); mHttpResponse = mHttpClient.execute(mHttpGet);
			 * mHttpEntity = mHttpResponse.getEntity(); mInputStream =
			 * mHttpEntity.getContent(); isResultOK = Geocoding(mInputStream,
			 * Constants.DAUM_LNG_TAG, Constants.DAUM_LAT_TAG,
			 * Constants.DAUM_ADDRESS_TAG); if(isResultOK){ if(Constants.D)
			 * Log.v(TAG, "DAUM OK"); return mAddress; }
			 */
			...
			isResultOK = Geocoding(mInputStream, Constants.NAVER_LNG_TAG,
					Constants.NAVER_LAT_TAG, Constants.NAVER_ADDRESS_TAG);
			if (isResultOK) {
				if (Constants.D)
					Log.v(TAG, "NAVER OK");
				if (Constants.D)
					Log.v(TAG, mAddress.getAddressLine(0));
				if (Constants.D)
					Log.v(TAG, "" + mAddress.getLatitude());
				if (Constants.D)
					Log.v(TAG, "" + mAddress.getLongitude());
				return mAddress;
			}
			...
			mHttpEntity = mHttpResponse.getEntity();
			mInputStream = mHttpEntity.getContent();
			isResultOK = Geocoding(mInputStream, Constants.GOOGLE_LNG_TAG,
					Constants.GOOGLE_LAT_TAG, Constants.GOOGLE_ADDRESS_TAG);
			if (isResultOK) {
				if (Constants.D)
					Log.v(TAG, "GOOGLE OK");
				if (Constants.D)
		...
		return null;
		/*
		 * try { mHttpResponse = mHttpClient.execute(mHttpGet); HttpEntity
		 * mHttpEntity = mHttpResponse.getEntity(); InputStream mInputStream =
		 * mHttpEntity.getContent(); int mReadByte; while ((mReadByte =
		 * mInputStream.read()) != -1) { mStringBuilder.append((char)
		 * mReadByte); } } catch (ClientProtocolException e) {
		 * 
		 * } catch (IOException e) {
		 * 
	...
	public static boolean Geocoding(InputStream inputStream, String LNG_TAG,
			String LAT_TAG, String ADDRESS_TAG) {
			...
			while (mEventType != XmlPullParser.END_DOCUMENT) {
				...
				case XmlPullParser.END_TAG:
					...
					if (tag.compareTo(LNG_TAG) == 0
							|| tag.compareTo(LAT_TAG) == 0
							|| tag.compareTo(ADDRESS_TAG) == 0) {
	 ...
	 * JSONObject mJsonObject = new JSONObject(); try { mJsonObject = new
	 * JSONObject(mAddressBuilder); } catch (JSONException e) {
	 * e.printStackTrace(); } Double mLongitude = new Double(0); Double
	 * mLatitude = new Double(0); String mFormattedAddress = null;
	 * 
	 * try { mFormattedAddress =
	 * ((JSONArray)mJsonObject.get("results")).getJSONObject(0)
	 * .getString("formatted_address");
	 * 
	 * //mDaumLong =
	 * ((JSONArray)jsonObject.get("item")).getJSONObject(0).getDouble("lng");
	 * 
	 ...
	 * e.printStackTrace(); } catch(ClassCastException e){ // TODO
	 * Auto-generated catch block e.printStackTrace(); }
	 * mAddress.setLongitude(mLongitude); mAddress.setLatitude(mLatitude);
	 * mAddress.setAddressLine(1, mFormattedAddress); return mAddress; }
	 */

	public static List<Address> getFromLocation(double lat, double lon,
			int maxResults) {
		String urlStr = "http://maps.google.com/maps/geo?q=" + lat + "," + lon
		...
		for (int i = 0; i < responseArray.length() && i < maxResults - 1; i++) {
			...
			try {
				 ...
				 * addy.setSubAdminArea
				 * (jsl.getString("SubAdministrativeAreaName"));
				 * 
				 * jsl = jsl.getJSONObject("Locality");
				 * addy.setLocality(jsl.getString("LocalityName"));
				 * 
				 * addy.setPostalCode(jsl.getJSONObject("PostalCode").getString(
				 * "PostalCodeNumber"));
